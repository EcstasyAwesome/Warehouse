...
public class EditCompany extends ConfiguredFeedbackController<Company> {

  private final WindowManager windowManager = WindowManager.getInstance();
  private final CompanyRepository companyRepository = CompanyRepositoryService.getInstance();
  private final Logger logger = LogManager.getLogger(EditCompany.class);
  ...
  private void save(ActionEvent event) {
    if (isFieldValid(nameField, result.getName(), NAME, companyRepository)
      ...
      var companyCopy = new Company(result);
      var contact = result.getBusinessContact();
      var address = result.getAddress();
      result.setName(nameField.getText());
      result.setIdentifierCode(identifierCodeField.getText());
      ...
      try {
        companyRepository.update(result);
        logger.info("Edited a company with id={}", result.getId());
        closeCurrentStage(event);
      } catch (NullPointerException exception) {
        result.setName(companyCopy.getName());
        result.setIdentifierCode(companyCopy.getIdentifierCode());
